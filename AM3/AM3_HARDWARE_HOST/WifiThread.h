...
const IPAddress AM3_HOST_AP_IP(192, 168, 4, 1);
const IPAddress AM3_HOST_AP_GATEWAY(192, 168, 4, 1);
const IPAddress AM3_HOST_AP_SUBNET(255, 255, 255, 0);

constexpr uint32_t AM3_ROUTER_RETRY_INTERVAL_MS = 10000UL;

unsigned long am3LastHostApHealthCheckMs = 0;
unsigned long am3LastHostNetworkStatusMs = 0;
unsigned long am3LastRouterRetryMs = 0;
bool am3HardwareApConfigured = false;
bool am3LastRouterConnected = false;
uint8_t am3LastDisplayStationCount = 255;

bool isHardwareHostAccessPointActive() {
  const wifi_mode_t mode = WiFi.getMode();
  const bool apModeEnabled = (mode == WIFI_AP || mode == WIFI_AP_STA);
  return apModeEnabled && WiFi.softAPIP() == AM3_HOST_AP_IP;
}
...
bool ensureHardwareHostAccessPoint() {
  ...
  if (am3HardwareApConfigured && isHardwareHostAccessPointActive()) {
    return true;
  }

  if (!WiFi.softAPConfig(AM3_HOST_AP_IP, AM3_HOST_AP_GATEWAY, AM3_HOST_AP_SUBNET)) {
  ...
  const bool started = WiFi.softAP(
    AM3_HARDWARE_AP_SSID,
    AM3_HARDWARE_AP_PASSWORD,
    channel,
    false,
    4
  );

  am3HardwareApConfigured = started;
...
bool serviceHardwareHostAccessPoint() {
  ...
  if (now - am3LastHostApHealthCheckMs < AM3_HOST_AP_HEALTH_CHECK_MS) {
    return false;
  }
  am3LastHostApHealthCheckMs = now;

  if (!isHardwareHostAccessPointActive()) {
    Serial.println("[HOST AP] Inactive; restarting AP");
    am3HardwareApConfigured = false;
    return ensureHardwareHostAccessPoint();
  }

  const uint8_t stationCount = WiFi.softAPgetStationNum();
  if (stationCount != am3LastDisplayStationCount) {
    am3LastDisplayStationCount = stationCount;
...
void serviceRouterWiFi() {
  ...
  if (connected != am3LastRouterConnected) {
    am3LastRouterConnected = connected;
  ...
  if (!connected && !ssid.isEmpty() &&
      now - am3LastRouterRetryMs >= AM3_ROUTER_RETRY_INTERVAL_MS) {
    am3LastRouterRetryMs = now;
    Serial.println("[ROUTER] Retrying station connection");
    WiFi.begin(ssid.c_str(), password.c_str());
  }

#if AM3_LINK_DEBUG
  if (now - am3LastHostNetworkStatusMs >= AM3_HOST_LINK_STATUS_INTERVAL_MS) {
    am3LastHostNetworkStatusMs = now;
