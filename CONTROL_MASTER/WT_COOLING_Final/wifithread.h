...
void DEVICE_INIT() {
  ...
  tmatched = preferences.getUInt("tmatched", 0);  // Load tmatched (default 0)
  timeMatched = (tmatched == 1);  // Sync timeMatched
...
void Extract_by_json(String incomingMessage) {
  ...
  if (newTmatched != tmatched) {
    ...
    preferences.putUInt("tmatched", tmatched);  // Save to preferences
    #ifdef DEBUG
...
void callback(char *topic, byte *message, unsigned int length) {
  ...
  if (String(topic) == device_topic_s) {  // /test/WTC-AAA011/1
#ifdef DEBUG
