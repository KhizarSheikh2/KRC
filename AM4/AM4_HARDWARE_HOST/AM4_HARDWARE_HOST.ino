...
// AM4 telemetry/config JSON packets are larger than PubSubClient's default.
...
void initAM4TaskWatchdog() {
#if ESP_ARDUINO_VERSION_MAJOR >= 3
  esp_task_wdt_config_t watchdogConfig = {};
  watchdogConfig.timeout_ms = 10000;
...
void getTemperature() {
  ...
  if (temp6Assigned) {
    OtherTempC = sensors.getTempC(temp6Address);
    if (isValidDS18B20Temperature(OtherTempC)) {
      OtherTempC += other_temp_offset;
      OtherTemp = convertToFahrenheit(OtherTempC);
    } else {
      OtherTempC = OtherTemp = SENSOR_DISCONNECTED;
    }
  } else {
    OtherTempC = OtherTemp = SENSOR_NOT_SELECTED;
  }
}
