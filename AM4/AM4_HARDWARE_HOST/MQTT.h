...
void publishJson() {
  ...
  doc["othertemp"] = String(OtherTempC, 1);
  doc["othertempF"] = String(OtherTemp);
  ...
  if (!client.publish(device_topic_p.c_str(), jsonBuffer, true)) {
    Serial.println("AM4 telemetry MQTT publish failed.");
  }
}

// Accept both int and narrower integer variables such as int16_t offsets.
// The previous int& parameter could not bind to offset1 ... offset6 because
// those variables are int16_t, producing a temporary int conversion.
template <typename TValue>
void update_values_from_json(const char* key, TValue& value_variable) {
  if (!received_doc.containsKey(key)) {
    return;
  }

  const int received_value = received_doc[key].as<int>();
  if (received_value == static_cast<int>(value_variable)) {
    return;
  }

  value_variable = static_cast<TValue>(received_value);
  preferences.putInt(key, received_value);
}
...
void temp_sensor_select_publish() {
  ...
  const int publishDeviceCount = (numberOfDevices > 6) ? 6 : numberOfDevices;
  ...
  const String device_topic_temp_config = device_topic_p + "/temperature_config_AM4";
...
void Extract_temp_config(String incoming_message) {
  ...
  // The app contract publishes offsets as offset1 ... offset6. They are
  // optional on /2, but when present they must be stored using these exact keys.
  update_values_from_json("offset1", offset1);
  update_values_from_json("offset2", offset2);
  update_values_from_json("offset3", offset3);
  update_values_from_json("offset4", offset4);
  update_values_from_json("offset5", offset5);
  update_values_from_json("offset6", offset6);
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == device_topic_s_1) {  // /test/AM4-AAA001/1
    Extract_by_json(messageTemp);
  }

  else if (String(topic) == device_topic_s_2) {  // /test/AM4-AAA001/2
    message_received_config = true;
    Extract_temp_config(messageTemp);
  }

  // else if (String(topic) == device_topic_s_3) {  // /test/AM4-AAA001/3
