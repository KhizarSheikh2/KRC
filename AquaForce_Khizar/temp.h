bool compareAddresses(DeviceAddress a, DeviceAddress b) {
  for (uint8_t i = 0; i < 8; i++) {
    if (a[i] != b[i]) return false;
  }
  return true;
}

void printAddress(DeviceAddress deviceAddress) {
  for (uint8_t i = 0; i < 8; i++) {
    if (deviceAddress[i] < 16) Serial.print("0");
    Serial.print(deviceAddress[i], HEX);
  }
  Serial.println();
}

void assignSensor(int sensor_sel, DeviceAddress sensorAddress) {
  preferences.begin("values", false);

  printAddress(sensorAddress);

  if (sensor_sel == 1) {
    memcpy(temp1Address, sensorAddress, sizeof(DeviceAddress));
    suction_temp_offset = offset1;
    temp1Assigned = true;
    preferences.putBool("temp1_assign", temp1Assigned);
    preferences.putBytes("temp1Address", sensorAddress, 8);
  } else if (sensor_sel == 2) {
    memcpy(temp2Address, sensorAddress, sizeof(DeviceAddress));
    temp2Assigned = true;
    discharge_temp_offset = offset2;
    preferences.putBool("temp2_assign", temp2Assigned);
    preferences.putBytes("temp2Address", sensorAddress, 8);
  } else if (sensor_sel == 3) {
    memcpy(temp3Address, sensorAddress, sizeof(DeviceAddress));
    temp3Assigned = true;
    subcool_temp_offset = offset3;
    preferences.putBool("temp3_assign", temp3Assigned);
    preferences.putBytes("temp3Address", sensorAddress, 8);
  } else if (sensor_sel == 4) {
    memcpy(temp4Address, sensorAddress, sizeof(DeviceAddress));
    temp4Assigned = true;
    spray_temp_offset = offset4;
    preferences.putBool("temp4_assign", temp4Assigned);
    preferences.putBytes("temp4Address", sensorAddress, 8);
  } else if (sensor_sel == 5) {
    memcpy(temp5Address, sensorAddress, sizeof(DeviceAddress));
    temp5Assigned = true;
    supply_temp_offset = offset5;
    preferences.putBool("temp5_assign", temp5Assigned);
    preferences.putBytes("temp5Address", sensorAddress, 8);
  } else if (sensor_sel == 6) {
    memcpy(temp6Address, sensorAddress, sizeof(DeviceAddress));
    temp6Assigned = true;
    return_air_temp_offset = offset6;
    preferences.putBool("temp6_assign", temp6Assigned);
    preferences.putBytes("temp6Address", sensorAddress, 8);
  }

  else if (sensor_sel == 0) {
    if (compareAddresses(sensorAddress, temp1Address)) {
      temp1Assigned = false;
      memset(temp1Address, 0x00, sizeof(temp1Address));
      preferences.putBool("temp1_assign", temp1Assigned);
      preferences.putBytes("temp1Address", temp1Address, 8);
    } else if (compareAddresses(sensorAddress, temp2Address)) {
      temp2Assigned = false;
      memset(temp2Address, 0x00, sizeof(temp2Address));
      preferences.putBool("temp2_assign", temp2Assigned);
      preferences.putBytes("temp2Address", temp2Address, 8);
    } else if (compareAddresses(sensorAddress, temp3Address)) {
      temp3Assigned = false;
      memset(temp3Address, 0x00, sizeof(temp3Address));
      preferences.putBool("temp3_assign", temp3Assigned);
      preferences.putBytes("temp3Address", temp3Address, 8);
    } else if (compareAddresses(sensorAddress, temp3Address)) {
      temp4Assigned = false;
      memset(temp4Address, 0x00, sizeof(temp4Address));
      preferences.putBool("temp4_assign", temp4Assigned);
      preferences.putBytes("temp4Address", temp4Address, 8);
    } else if (compareAddresses(sensorAddress, temp5Address)) {
      temp5Assigned = false;
      memset(temp5Address, 0x00, sizeof(temp5Address));
      preferences.putBool("temp5_assign", temp5Assigned);
      preferences.putBytes("temp5Address", temp5Address, 8);
    } else if (compareAddresses(sensorAddress, temp6Address)) {
      temp6Assigned = false;
      memset(temp6Address, 0x00, sizeof(temp6Address));
      preferences.putBool("temp6_assign", temp6Assigned);
      preferences.putBytes("temp6Address", temp6Address, 8);
    }
  }

  preferences.end();
}

void assignSensorB(int sensor_sel, DeviceAddress sensorAddress) {
  preferences.begin("valuesB", false);

  printAddress(sensorAddress);

  if (sensor_sel == 1) {
    memcpy(temp7Address, sensorAddress, sizeof(DeviceAddress));
    temp7Assigned = true;
    suction_temp_offset2 = offset7;
    preferences.putBool("temp1_assign", temp7Assigned);
    preferences.putBytes("temp1Address", sensorAddress, 8);
  } else if (sensor_sel == 2) {
    memcpy(temp8Address, sensorAddress, sizeof(DeviceAddress));
    temp8Assigned = true;
    discharge_temp_offset2 = offset8;
    preferences.putBool("temp2_assign", temp2Assigned);
    preferences.putBytes("temp2Address", sensorAddress, 8);
  } else if (sensor_sel == 3) {
    memcpy(temp9Address, sensorAddress, sizeof(DeviceAddress));
    temp9Assigned = true;
    subcool_temp_offset2 = offset9;
    preferences.putBool("temp3_assign", temp9Assigned);
    preferences.putBytes("temp3Address", sensorAddress, 8);
  } else if (sensor_sel == 4) {
    memcpy(temp10Address, sensorAddress, sizeof(DeviceAddress));
    temp10Assigned = true;
    spray_temp_offset2 = offset10;
    preferences.putBool("temp4_assign", temp10Assigned);
    preferences.putBytes("temp4Address", sensorAddress, 8);
  } 

  else if (sensor_sel == 0) {
    if (compareAddresses(sensorAddress, temp7Address)) {
      temp7Assigned = false;
      memset(temp7Address, 0x00, sizeof(temp7Address));
      preferences.putBool("temp1_assign", temp7Assigned);
      preferences.putBytes("temp1Address", temp7Address, 8);
    } 
    else if (compareAddresses(sensorAddress, temp8Address)) {
      temp8Assigned = false;
      memset(temp8Address, 0x00, sizeof(temp8Address));
      preferences.putBool("temp2_assign", temp8Assigned);
      preferences.putBytes("temp2Address", temp8Address, 8);
    } 
    else if (compareAddresses(sensorAddress, temp9Address)) {
      temp9Assigned = false;
      memset(temp9Address, 0x00, sizeof(temp9Address));
      preferences.putBool("temp3_assign", temp9Assigned);
      preferences.putBytes("temp3Address", temp9Address, 8);
    } else if (compareAddresses(sensorAddress, temp10Address)) {
      temp10Assigned = false;
      memset(temp10Address, 0x00, sizeof(temp10Address));
      preferences.putBool("temp4_assign", temp10Assigned);
      preferences.putBytes("temp4Address", temp10Address, 8);
    }
  }

  preferences.end();
}

String getAddressString(DeviceAddress deviceAddress) {
  String address = "";
  for (uint8_t i = 0; i < 8; i++) {
    if (deviceAddress[i] < 16) address += "0";
    address += String(deviceAddress[i], HEX);
  }
  return address;
}

void setoffset(DeviceAddress tempSensorAddresses, int16_t offset_received) {
  if (compareAddresses(tempSensorAddresses, temp1Address)) {
    suction_temp_offset = offset_received;
  } else if (compareAddresses(tempSensorAddresses, temp2Address)) {
    discharge_temp_offset = offset_received;
  } else if (compareAddresses(tempSensorAddresses, temp3Address)) {
    subcool_temp_offset = offset_received;
  } else if (compareAddresses(tempSensorAddresses, temp4Address)) {
    spray_temp_offset = offset_received;
  } else if (compareAddresses(tempSensorAddresses, temp5Address)) {
    supply_temp_offset = offset_received;
  } else if (compareAddresses(tempSensorAddresses, temp6Address)) {
    return_air_temp_offset = offset_received;
  } else if (compareAddresses(tempSensorAddresses, temp7Address)) {
    suction_temp_offset2 = offset_received;
  } else if (compareAddresses(tempSensorAddresses, temp8Address)) {
    discharge_temp_offset2 = offset_received;
  } else if (compareAddresses(tempSensorAddresses, temp9Address)) {
    subcool_temp_offset2 = offset_received;
  } else if (compareAddresses(tempSensorAddresses, temp10Address)) {
    spray_temp_offset2 = offset_received;
  }
}

void read_temp(float& temp, DeviceAddress deviceaddress) {
  sensors.setWaitForConversion(false);
  sensors.requestTemperatures();
  sensors.setWaitForConversion(true);
  temp = sensors.getTempC(deviceaddress);
}

void read_tempB(float& temp, DeviceAddress deviceaddress) {
  sensorsB.setWaitForConversion(false);
  sensorsB.requestTemperatures();
  sensorsB.setWaitForConversion(true);
  temp = sensors.getTempC(deviceaddress);
}
